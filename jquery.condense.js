 ...
 *  fixHeight: fix height. Default: "false"
 */
...
(function($) {
    ...
    $.fn.condense = function(options) {
        ...
        return this.each(function() {
            ...
            if (o.fixHeight) {
                ...
                var $i = 0;
                while(  $this[0].scrollHeight > $this.height() && $i < 100) {
                    $this.text($this.text().slice(0, $originText.length - $i ));
                    $i ++;
                }
                if ($i > 0) {
                    $this.text($originText.slice(0,$originText.length - $i- o["ellipsis"].length ) + o["ellipsis"]);
                }
                debug("OriginTextLength:" + $originText.length + ", CondensingTextLength:" + $this.text().length);
    ...
    function cloneCondensed(elem, opts){
        ...
        if ($.trim(elem.text()).length <= opts.condensedLength + opts.minTrail){
            debug('element too short: skipping.(condensedLength:' +  opts.condensedLength + ',minTrail:' + opts.minTrail + ')');
            return false;
        }
